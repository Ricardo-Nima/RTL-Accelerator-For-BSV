        ...
        if (vec.size() > 2)
        {
            if (vec[0] == "class" && vec[1] == "MOD_" + cur_module->cur.module_name)
            {
                ...
                for (auto& relation : en_regs_relation)
                {
                    ...
                    g_modified_file += "\t\t\t\tout_post += \"\\t#\" + std::to_string(temp) + \" \" + \"" + string("EN_") + relation.first + "\" + \" = \" + \"1\" + \";\\n\";\n";
                ...
                g_modified_file += "\t\tout_post += \"initial begin\\n\";\n";
                g_modified_file += "\t\tout_post += \"\\t$dumpfile(\\\"\" + cur_hierachy + \".vcd\\\");\\n\";\n";
                g_modified_file += "\t\tout_post += \"\\t$dumpvars(0, testbench);\\n\";\n";
                g_modified_file += "\t\tout_post += \"end\\n\";\n";
...
{
    ifstream fin("../src/zjout/" + cur_module->cur.module_name + ".h");
    insert_header(fin);
    insert_change_lists(fin, cur_module);
    string outfile = "../src/zjout/" + cur_module->cur.module_name + ".h";
...
{
    ifstream fin("../src/zjout/" + cur_module->cur.module_name + ".cxx");
    string line;
    vector<string> vec;
    bool hiera = 0;
    while (getline(fin, line))
    ...
    string outfile = "../src/zjout/" + cur_module->cur.module_name + ".cxx";
    ofstream fout(outfile);
    fout << g_modified_file;
}
...
{
    g_modified_file = "";
    ifstream fin("../src/zjout/main.cpp");
    string line;
    vector<string> vec;
    bool hiera = 0;
    while (getline(fin, line))
    ...
    ofstream fout("../src/zjout/main.cpp");
    fout << g_modified_file;
}
void bs_til_n()
{
...
{
    g_modified_file = "";
    ifstream fin("../src/zjout/model_mkTb.h");
    string line;
    vector<string> vec;
    bool hiera = 0;
    while (getline(fin, line))
    ...
    ofstream fout("../src/zjout/model_mkTb.h");
    fout << g_modified_file;
}
...
bool change_verilog(module_hierachy *cur_module)
{
    ...
    g_modified_file = "";
    ifstream fin("../src/zjout/" + cur_module->cur.module_name + ".v");
    string line;
    vector<string> vec;
    bool in_comment = 0;
    while (getline(fin, line))
    ...
    ofstream fout("../src/zjout/" + cur_module->cur.module_name + ".v");
    fout << g_modified_file;
}
int main()
{
